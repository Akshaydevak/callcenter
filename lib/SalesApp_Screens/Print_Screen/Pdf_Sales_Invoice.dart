...
class PrintScreenInvoice extends StatefulWidget {
  ...
  final  List<OrderLineInvoice> table;
  ...
  @override
  _PrintScreenInvoiceState createState() => _PrintScreenInvoiceState();
}

class _PrintScreenInvoiceState extends State<PrintScreenInvoice> {
  ...
  listener: (context, state) {
    state.maybeWhen(
        ...
        success: (data) {
          setState(() {
            readData=data;
            print("READING INVOICE$readData");
  ...
  child: Scaffold(
      ...
      body:PdfPreview(
        build: (format) => _generatePdf(format,"title",widget.orderDate, widget.orderCode,context,widget.vendorCode,
            widget.discount,widget.totalPrice,widget.taxableAmount,widget.unitCost,widget.excisetax,widget.vat,widget.note,widget.remarks,widget.table,
        Mobile,Email),
      ),
...
Future<Uint8List> _generatePdf(PdfPageFormat format, String title,String orderDate,String orderCode,BuildContext context,String vendorCode,
    ...
    ,double? excisetax,double? vat,String note,String remarks,List<OrderLineInvoice> table,String mobile,String email) async {
  double height = MediaQuery.of(context).size.height;
  double width = MediaQuery.of(context).size.width;
